...
class Parser
{
          ...
          $curLow != "{"
            ...
            if ($this->varCheck($curTok) || $curLow == '*') {
                if ($bWaitForRenaming) {
                    ...
                    $currentVar = new ResultVariable($curTok);
                    if ($currentFunc != null) {
                        $currentVar->setFunc($currentFunc);
                    }
                }
    ...
    protected function parseFrom()
    {
        ...
        if(strtolower(current($this->tokens))!='named'){
            if($this->iriCheck(current($this->tokens))||$this->qnameCheck(current($this->tokens))){
                ...
                throw new ParseException("Variable, Iri or qname expected in FROM ",null,key($this->tokens));
            }
            $this->query->addFrom(current($this->tokens));
        }else{
            $this->_fastForward();
            if($this->iriCheck(current($this->tokens))||$this->qnameCheck(current($this->tokens))){
                ...
                throw new ParseException("Variable, Iri or qname expected in FROM NAMED ",null,key($this->tokens));
            }
        }
    }
    ...
    protected function parseConstruct()
    {
        ...
        if(current($this->tokens)=="{"){
            ...
            throw new ParseException("Unable to parse CONSTRUCT part. '{' expected. ",null,key($this->tokens));
        }
        $this->parseWhere();
        $this->parseModifier();
    }
    ...
    protected function parseWhere()
    {
        ...
        if(current($this->tokens)=="{"){
            ...
            throw new ParseException("Unable to parse WHERE part. '{' expected in Query. ",null,key($this->tokens));
        }
    }
    ...
    protected function qnameCheck($token)
    {
        ...
        if (preg_match($pattern,$token,$hits)>0) {
            ...
            throw new ParseException("Unbound Prefix: <i>".$hits{1}."</i>",null,key($this->tokens));
        } else {
            return false;
        }
    }
    ...
    protected function parseGraph()
    {
        ...
        if(!$this->varCheck($name)&!$this->iriCheck($name)&&!$this->qnameCheck($name)){
            $msg = $name;
            $msg = preg_replace('/</', '&lt;', $msg);
            throw new ParseException(" IRI or Var expected. ",null,key($this->tokens));
        }
        $this->_fastForward();
            ...
            switch(strtolower(current($this->tokens))){
                case "order":
                    ...
                    if(strtolower(current($this->tokens))=='by'){
                        ...
                        throw new ParseException("'BY' expected.",null,key($this->tokens));
                    }
                    break;
                case "limit":
                    $this->_fastForward();
            ...
            switch (strtolower(current($this->tokens))) {
                case "desc":
                    ...
                    if($this->varCheck(current($this->tokens))) {
                        ...
                        throw new ParseException("Variable expected in ORDER BY clause. ",null,key($this->tokens));
                    }
                    $this->_fastForward();
                    if(current($this->tokens)!=')')
                        throw new ParseException("missing ')' in ORDER BY clause.",null,key($this->tokens));
                    $val['type'] = 'desc';
                    $this->_fastForward();
                    break;
                case "asc" :
